...
export default class Bot 
{
  ...
  */
  ...
  ) 
  {
    ...
    if (card != "None" && urls[0] == "None") // Check if there is a link card, and there's not at least one url. This might be implemented incorrectly.
    {
      ...
      if (cards[3] != "None")
      {
        ...
        if ((cardBuffer != undefined && cardBuffer.length > 1000000) || cards[3] == "None")
        {
          ...
          var cardResponse = await axios.get("https://media.bizj.us/view/img/12050561/frank-seravalli*1200xx1920-1083-0-148.jpg", { responseType: 'arraybuffer'}); 
          cardBuffer = Buffer.from(cardResponse.data, "utf-8");
        }
    ...
    var bskyFeedAwait = await this.#agent.getAuthorFeed({actor: "notfrankseravalli.bsky.social", limit: postNum,}); // Get a defined number + 2 of most recent posts from the logged in user's feed.
